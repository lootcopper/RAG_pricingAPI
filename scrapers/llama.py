...
class LlamaScraper(BaseProviderModelScraper):
    def __init__(self):
        super().__init__()
        self.provider_name = "AWS Bedrock"
        self.base_url = "https://aws.amazon.com/bedrock/pricing/" e

    def scrape(self) -> List[ProviderModelSpec]:
        print(f"Scraping Llama models from {self.base_url}")
    ...
    def _extract_from_sections(self, soup: BeautifulSoup) -> List[Dict]:
        ...
        for section in pricing_sections:
            ...
            if not self._is_llama_section(text_content):
                continue

           
            llama_patterns = [
                r'llama[\s-]*(?:\d+\.?\d*)?[a-z]*(?:[\s-]*(?:chat|instruct|code))?',
                r'llama[\s-]*\d+[a-z]*',
                r'meta[\s-]*llama[\s-]*\d+[a-z]*'
            ]
    ...
    def _is_llama_section(self, text: str) -> bool:
        """Check if the text section contains Llama model information"""
        text_lower = text.lower()
        return ('llama' in text_lower or 'meta' in text_lower) and (
            'pricing' in text_lower or 
            '$' in text or 
            'tokens' in text_lower or
            'input' in text_lower or
            'output' in text_lower
        )

    def _clean_model_name(self, raw_name: str) -> Optional[str]:
        unwanted_phrases = [
            'This is some text inside of a div block.',
            'try it', 'ChatLlama', 'Chat', '→',
        ...
        if 'llama' in cleaned.lower():
            ...
            if 'meta' in cleaned.lower():
                cleaned = re.sub(r'meta[\s-]*llama', 'Meta Llama', cleaned, flags=re.IGNORECASE)
            else:
                cleaned = re.sub(r'llama', 'Llama', cleaned, flags=re.IGNORECASE)
        ...
        if (len(cleaned) < 3 or len(cleaned) > 50 or 
            ...
            cleaned.lower() in ['llama', 'meta llama']):
            return None
        ...
        for pattern in input_patterns:
            ...
            if match:
                try:
                    ...
                    if 'per 1000' in text.lower() or 'per 1k' in text.lower():
                        price *= 1000
                    pricing['input_price_per_1m_tokens'] = price
                    break
                except ValueError:
                    continue
        ...
        for pattern in output_patterns:
            ...
            if match:
                try:
                    ...
                    if 'per 1000' in text.lower() or 'per 1k' in text.lower():
                        price *= 1000
                    pricing['output_price_per_1m_tokens'] = price
                    break
                except ValueError:
                    continue
